...
for file in files:
    ...
    ocrGoogle = open(os.path.join(dir, f"ocr_{pageNum}.txt")).read().replace("[", "").replace("]", "").replace("—", "")
    ocrGoogle = ocrGoogle.split('\n')

    ocrTesseract = ''
    if(os.path.exists(os.path.join(dir, f"ocr_tesseract_{pageNum}.txt"))):
    	ocrTesseract = open(os.path.join(dir, f"ocr_tesseract_{pageNum}.txt")).read().replace("[", "").replace("]", "").replace("—", "").split("\n")
    # break ocrText into lines of 7 words and connect it into a whole text
    ocr = [transform(o.split()) for o in ocrGoogle]
    correctedText1 = [correctOcr(i)  for o in ocr for i in o]
    correctedText1 = " ".join(correctedText1)
    open(f"{file_path}.corrected.txt", "w", encoding="utf-8").write(correctedText1)
    ocr_t = [transform(o.split()) for o in ocrTesseract]
    correctedText2 = [correctOcr(i) for i in o for o in ocr_t]
    correctedText2 = " ".join(correctedText2)
    ocrGoogle = ocrGoogle.replace('\n', ' ')
    ocrTesseract = ocrTesseract.replace('\n', ' ')
